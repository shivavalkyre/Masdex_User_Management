...
const create = (request, response) => {
    ...
    pool.query('INSERT INTO tbl_role (role) VALUES ($1) RETURNING id', [role], (error, results) => {
        if (error) {
          if (error.code == '23505')
          {
...
const update = (request, response) => {
    ...
    pool.query('SELECT * FROM tbl_role WHERE id=$1', [id],(error, results) => {
      ...
      if (results.rowCount >0){
        ...
        pool.query('UPDATE tbl_role set role=$1,updated_at=$2 WHERE id=$3', [role,update_time,id], (error, results) => {
          if (error) {
            response.status(400).send({success:false,data: error})
            return;
          }
